...
const BookingsPage = () => {
  ...
  const fetchBookings = async () => {
    try {
      const { data } = await axios.get(BASE_URL + "/bookings/my", {
  ...
  const downloadReceiptHandler = async (bookingId) => {
    try {
      const response = await axios.get(
        `${BASE_URL}/bookings/receipt/${bookingId}`,
        {
          responseType: "blob",
          withCredentials: true,
        }
      ...
      console.log("Receipt download failed:", error);
      alert("Receipt not available for this booking.");
    }
  };
